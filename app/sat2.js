...
internal.Heatmap = class {
    ...
    toFWC() {
		...
		for(var j=0;j<this.equipos.length;j++) {
			tab += sprintf("%-22s",this.equipos[j].substring(0,22)) + "" + rows[j].map(z=> sprintf("%"+colwidth+"s",z)).join("") + "\n"
		}
		return tab
	}
	...
	toCSV(sep=";") {
		...
		for(var j=0;j<this.equipos.length;j++) {
			csv += this.equipos[j].substring(0,22) + sep + rows[j].join(sep) + "\n"
		}
		return csv
	}
}
...
internal.CRUD = class{
	...
	readEquipos(idEquipo,orderBy,idGrupo) {
		return new Promise( (resolve, reject) => {
			...
			//~ console.log("filter")
			//~ console.log(filter)
			this.pool.query("SELECT equipos.\"idEquipo\", equipos.descripcion, st_y(geom) AS lng, st_x(geom) AS lat, \"NroSerie\", to_char(\"fechaAlta\", 'YYYY-MM-DD\"T\"HH24:MI:SS')  AS \"fechaAlta\" , json_agg(json_build_object('idSensor', sensores.\"idSensor\", 'nombre', sensores.\"nombre\", 'Icono', sensores.\"Icono\")) AS sensores, equipos.\"idGrupo\" \
	...
	readDatos(idEquipo,idSensor,fechaInicial, fechaFinal, orderBy) {
		return new Promise( (resolve, reject) => {
			// idSensor int|int[]|string default null
	...
	readAsociaciones(idEquipo,idSensor,idGrupo) {
		return new Promise( (resolve, reject) => {
			var filter = ""
			if(Array.isArray(idEquipo)) {
				for(var i=0; i<idEquipo.length;i++) {
	...
	readHeatmap(heatmap, use = 'id', format='json', sep=";") {
	...
	readGrupos(idGrupo) {
		return new Promise( (resolve, reject) => {
			...
			this.pool.query("SELECT \"idGrupo\", descripcion FROM grupos " + filter)
			.then(res=>{
				var grupos=[]
				if(res.rows) {
					for(var i=0; i<res.rows.length;i++) {
